...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!candidate)
        [candidate] = await UserAuth.findAll({ where: { username } });
      if (candidate) {
        return res
          .status(400)
      ...
      res
        .status(500)
        .json({ message: "Что-то пошло не так, попробуйте снова", error: e });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!user)
        [user] = await UserAuth.findAll({ where: { username: email } });
